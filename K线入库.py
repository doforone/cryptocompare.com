...
if __name__ == "__main__":
    ...
    for aa in aaa:
        for bb in bbb:
            ...
            havee=1
            while havee==1:
                t = threading.Thread(target=get_htmll,args=(f"https://min-api.cryptocompare.com/data/v2/histoday?fsym={aa}&tsym={bb}&allData=true",))
                t.setDaemon(True)
                t.start()
                t.join(33)
                if t.is_alive ():
            ...
            with open(f'data/{aa}_{bb}_d.txt', 'w', encoding='utf-8', newline='\r\n') as f:
