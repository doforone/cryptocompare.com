...
if __name__ == "__main__":
    ...
    for uu in uuu:
        ...
        # 定义要操作的文件夹路径
        folder_path = f"data/{uu[2]}/"  # 将这里替换为你实际想要创建的文件夹路径，比如 "test_folder"
        ...
        if not os.path.exists(f'data/{uu[2]}/{uu[0]}_{uu[1]}_d.txt'):
            print(uu)
            ddd=[]
            havee=1
            while havee==1:
                t = threading.Thread(target=get_htmll,args=(f"https://min-api.cryptocompare.com/data/v2/histoday?fsym={uu[0]}&tsym={uu[1]}&allData=true&e={uu[2]}",))
                t.setDaemon(True)
                t.start()
                t.join(33)
                if t.is_alive ():
                    ...
                    try:
                        jjj=json.loads(htmll)["Data"]["Data"]
                        for jj in jjj[:-1]:
                            ddd.append([jj["time"], jj["open"], jj["low"], jj["high"], jj["close"], jj["volumefrom"], jj["volumeto"]])
                        havee=0
                    except Exception as e:
                        print(e)
                        pass
                    
            with open(f'data/{uu[2]}/{uu[0]}_{uu[1]}_d.txt', 'w', encoding='utf-8', newline='\r\n') as f:
